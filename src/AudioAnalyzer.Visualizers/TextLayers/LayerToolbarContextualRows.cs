...
public static class LayerToolbarContextualRows
{
    ...
    public static IReadOnlyList<LayerToolbarContextualRow> Resolve(TextLayerSettings layer, int snippetIndex, UiSettings? uiSettings = null)
    {
        return layer.LayerType switch
        {
            TextLayerType.Oscilloscope => ResolveOscilloscope(layer),
            TextLayerType.AsciiImage => ResolveAsciiImage(layer, snippetIndex, uiSettings),
            TextLayerType.AsciiModel => ResolveAsciiModel(layer, snippetIndex, uiSettings),
            _ => s_empty
        };
    }
    ...
    private static IReadOnlyList<LayerToolbarContextualRow> ResolveAsciiImage(TextLayerSettings layer, int snippetIndex, UiSettings? uiSettings)
    {
        _ = snippetIndex;
        var s = layer.GetCustom<AsciiImageSettings>() ?? new AsciiImageSettings();
        var paths = FileBasedLayerAssetPaths.GetSortedImagePaths(s.ImageFolderPath, uiSettings);
        if (paths.Count == 0)
        {
    ...
    private static IReadOnlyList<LayerToolbarContextualRow> ResolveAsciiModel(TextLayerSettings layer, int snippetIndex, UiSettings? uiSettings)
    {
        _ = snippetIndex;
        var s = layer.GetCustom<AsciiModelSettings>() ?? new AsciiModelSettings();
        var paths = FileBasedLayerAssetPaths.GetSortedObjPaths(s.ModelFolderPath, uiSettings);
        if (paths.Count == 0)
        {
