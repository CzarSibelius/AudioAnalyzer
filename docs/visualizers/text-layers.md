...
- **TextLayers.Layers** (array): Each layer has:
  ...
  - Optional **`RenderBounds`**: `{ "X", "Y", "Width", "Height" }` (0–1, relative to the visualizer viewport). **Omit or null** = full viewport. Outside the rectangle, lower layers show through. See [ADR-0058](../adr/0058-layer-render-bounds.md).
