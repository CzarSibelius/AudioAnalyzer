...
3. **Selected palette and cycling**: **`AppSettings.SelectedPaletteId`** stores the current palette id. On startup, the app resolves the palette (from repository or, for backward compatibility, from `VisualizerSettings.UnknownPleasures.Palette`). The user cycles palettes with key **P**; the selected id is persisted on save. Palette-aware visualizers (those with **`SupportsPaletteCycling`** true, e.g. Geiss and Unknown Pleasures) receive the resolved palette via the snapshot; the toolbar shows the current palette name when applicable.
