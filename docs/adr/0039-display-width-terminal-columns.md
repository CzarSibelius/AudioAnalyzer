...
3. **Scrolling and truncation use display width**: ScrollingTextEngine, ScrollingTextViewport.RenderWithLabel, ScrollingTextComponentRenderer and HorizontalRowComponentRenderer (title, header, toolbar, and settings hint rows), and StaticTextViewport.TruncateToWidth/TruncateWithEllipsis use display width (terminal columns) when fitting text. Scroll offset and slice width are in column units.
