...
`AnalysisSnapshot` is produced by `AnalysisEngine.GetSnapshot()` for analysis data, while the visualization orchestrator mutates a **per-frame** snapshot for display (`FrameDeltaSeconds`, `MeasuredMainRenderFps`). The settings modal uses `GetSnapshotForUi()`, which historically returned only the engine snapshot, so values written during `TextLayersVisualizer.Render` were not visible to the modal.
...
3. **Snapshot field**: `AnalysisSnapshot` exposes `LayerRenderTimeMs`: `double?[]?` with length `TextLayersLimits.MaxLayerCount`, index = **sorted layer index**. `null` at an index means that layer was not timed this frame (disabled, missing renderer, or setting off). Values are **display/render pipeline** data, not from the analysis engine (ADR-0024).

4. **Orchestrator cache**: After each successful `_renderer.Render(snapshot)`, copy `LayerRenderTimeMs` into an orchestrator-held buffer when the setting is on. `GetSnapshotForUi()` returns `GetSnapshot()` from the engine and **merges** a **copy** of that cached array into the result so the modal never mutates the cache during the next frame.
...
6. **Toolbar**: Not in scope: the toolbar is drawn before the visualizer in Preset editor, so same-frame timings would require staging; only the modal shows timings via `GetSnapshotForUi()`.

## Consequences

- Layer timings reflect **last completed main render** when read from `GetSnapshotForUi()`, which is sufficient for debugging.
