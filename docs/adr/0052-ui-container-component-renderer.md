...
- All single-line UI (title bar, header rows, toolbar) uses the same leaf type (LabeledRowComponent) and ILabeledRowRenderer; only the block region (VisualizerAreaComponent) has a dedicated renderer. The "generic" layer is the single row component plus the dispatcher.
- ADR-0051 is preserved: Viewport remains data; labeled rows still use ILabeledRowRenderer with viewport lists.
