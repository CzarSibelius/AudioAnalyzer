...
2. **VisualizerSettings and per-visualizer types in Visualizers**: `VisualizerSettings`, `TextLayerSettings`, `TextLayersVisualizerSettings`, `GeissVisualizerSettings`, `OscilloscopeVisualizerSettings`, `UnknownPleasuresVisualizerSettings`, and `TextLayerType` live in the Visualizers project (or Domain where moved per ADR-0015). Beat reaction is layer-specific: each layer type that supports it has its own enum and stores it in Custom (see [ADR-0055](0055-layer-specific-beat-reaction.md)). Each per-visualizer settings type resides in its visualizer subfolder (e.g. `TextLayers/`, `Geiss/`).
