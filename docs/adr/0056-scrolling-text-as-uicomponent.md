...
4. **Label formatting**: Shared **LabelFormatting.FormatLabel(label)** in Application.Display. Used by ScrollingTextComponentRenderer, **ScrollingTextViewport**, and **TextLayersToolbarBuilder**. TextLayersToolbarBuilder no longer depends on **IScrollingTextViewport** for label punctuation; it uses LabelFormatting only.
