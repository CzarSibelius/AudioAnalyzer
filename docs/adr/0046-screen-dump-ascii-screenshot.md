...
- **Capture**: Capture is abstracted by **`IScreenDumpContentProvider`** (in `Application.Abstractions`) and provided per platform via DI ([ADR-0092](0092-platform-behavior-via-abstractions-and-di-module.md)): Windows uses `WindowsConsoleScreenDumpContentProvider` (in `AudioAnalyzer.Platform.Windows`), which reads the console screen buffer via P/Invoke (`ReadConsoleOutputCharacterW`, `GetConsoleScreenBufferInfo`) and captures exactly what is on screen without changing the render path; macOS uses `NullScreenDumpContentProvider`. On API failure or unsupported platforms the provider returns null (graceful degradation).
