...
TextLayers presets are stored as **JSON files** in a `**presets`** directory next to the executable (e.g. `presets/` in the same folder as the .exe). Each file is one preset. The app ships with one **`showcase-<visualizer>.json`** preset per built-in visualizer (e.g. `showcase-oscilloscope.json`, `showcase-starfield.json`, `showcase-geiss-background.json`), each showing that single layer over a black `Fill` background. Press **V** to cycle presets; **S** to edit (R rename, N new preset). Presets are created automatically on first run.
...
- **Logging** (`Logging`, per [ADR-0076](adr/0076-configurable-application-logging.md), [ADR-0083](adr/0083-concurrent-process-instances-and-log-files.md)): Optional file logging for diagnostics. `**Enabled`** (`bool`, default `false`): when `true`, log lines at or above `**MinimumLevel**` are appended to `**FilePath**`. `**FilePath**` (optional string): relative paths are under the **writable user-data root** — on macOS app bundles this is `~/Library/Application Support/AudioAnalyzer` (same place as `appsettings.json`, presets, palettes); on other hosts it is the application base directory (next to the executable). When omitted or empty, defaults to `logs/audioanalyzer-{ProcessId}.log` under that root (the `{ProcessId}` token is expanded once at startup to the current process id so multiple instances do not contend for one file). You may include `{ProcessId}` anywhere in a relative or absolute path for the same effect. If you set a **single fixed path without** `{ProcessId}`, a second process may fail to open the file on Windows or lines may interleave; use that only for single-instance runs or when an external tool merges logs. `**MinimumLevel`** (string): `Trace`, `Debug`, `Information`, `Warning`, `Error`, or `Critical` (same names as `Microsoft.Extensions.Logging.LogLevel`); invalid values are treated as `Error`. Default when omitted: `**Error**`. Log files may contain paths, device names, or other environment-specific text; enable only when you accept that privacy trade-off. Restart the app after editing this section (or reload settings the same way as other `appsettings.json` changes). **ASCII video (webcam)** failures and empty-device cases are logged under category `**AudioAnalyzer.Platform.Windows.AsciiVideo.WindowsAsciiVideoFrameSource`** (use `**Warning**` or lower to see them; `**Debug**` includes resolution-cap attempts).
...
```json
{
  ...
  "Logging": {
    "Enabled": false,
    "FilePath": "logs/audioanalyzer-{ProcessId}.log",
    "MinimumLevel": "Error"
  },
  "UiSettings": {
  ...
  "VisualizerSettings": {
    "ActivePresetId": "showcase-oscilloscope",
