...
**Now-playing (header `Now:` row + `NowPlaying` layer, macOS):** Build the **`MediaRemoteAdapter.framework`** ([`native/README.md`](../native/README.md), [docs/configuration-reference.md](configuration-reference.md)) so the finalize step copies it and **`mediaremote-adapter.pl`** into **`Contents/Resources/mediaremote-adapter/`**. The app then shows what another app (Music, Spotify, a browser, …) is playing — **Artist - Title** (and **Album** on macOS). **No** privacy prompt is involved (now-playing read needs no TCC consent). Without the artifacts macOS falls back to an empty `Now:` row ([ADR-0094](adr/0094-macos-now-playing-mediaremote-adapter.md)).
