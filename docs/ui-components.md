...
| **SettingsModal** | ISettingsModal | Overlay modal. Layer/preset/settings editing; uses ISettingsModalRenderer + IKeyHandler&lt;SettingsModalKeyContext&gt;. Idle: **onIdleVisualizationTick** (`Redraw`) then **DrawIdleOverlayTick** when needed. Hint via **HorizontalRowComponent** (ADR-0057). Palette rows/picker via **SettingsSurfacesPaletteDrawing**. UI spec: [preset settings modal](../specs/console-ui/preset-settings-modal/spec.md). ([SettingsModal.cs](../src/AudioAnalyzer.Console/Console/SettingsModal.cs), [SettingsModalRenderer.cs](../src/AudioAnalyzer.Console/SettingsModal/SettingsModalRenderer.cs)) |
