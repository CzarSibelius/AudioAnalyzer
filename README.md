...
- **UI settings** (`UiSettings`, per [ADR-0033](docs/adr/0033-ui-principles-and-configurable-settings.md)): App title, optional `TitleBarAppName` (short name for title bar, e.g. "aUdioNLZR"), optional `TitleBarPalette` (colors for title bar segments), UI palette (Normal, Highlighted, Dimmed, Label, optional Background), and default scrolling speed. Stored in `appsettings.json`. Colors support 24-bit RGB (`R`, `G`, `B`) or console color names (`Value`).
