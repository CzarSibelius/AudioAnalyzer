...
- **Superseded ADRs**: Check status in `docs/adr/README.md` and each ADR header. For **Superseded**, do not follow historical Decision sections; follow **Update (current)** / **Consequences (current)**, any **Superseded by** link, and **canonical** ADRs named there or in the index. For **Accepted** ADRs that say a detail is superseded, follow the newer cross-referenced ADR for that detail.
...
- **Viewport labels**: Use `Label:` only in labeled UI; do not embed hotkeys in labels. Use LabelFormatting / IScrollingTextViewport.FormatLabel for punctuation; key discovery is via help (H) and GetBindings() (see ADR-0033 and ADR-0049 in docs/adr/; ADR-0034 is historical only).
